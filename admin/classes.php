...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save') {
        ...
        if (empty($name) || !$levelId) {
            ...
            dbExecute(
                "UPDATE classes SET name=?, level_id=?, option_id=?, grade_year=?, section=?, capacity=?, room=?, class_teacher_id=?
                 WHERE id=?",
                [$name, $levelId, $optionId, $gradeYear, $section, $capacity, $room ?: null, $teacherId, $id]
            );
            logActivity('class_updated', "Classe modifiee : $name");
            redirectWith($_SERVER['PHP_SELF'], 'success', 'Classe modifiee.');
        } else {
            dbExecute(
                "INSERT INTO classes (academic_year_id, level_id, option_id, name, grade_year, section, capacity, room, class_teacher_id)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                [$yearId, $levelId, $optionId, $name, $gradeYear, $section, $capacity, $room ?: null, $teacherId]
            );
...
$classes = dbFetchAll(
    "SELECT c.*, l.name AS level_name, o.name AS option_name,
        ...
        <div class="card-body">
          <div class="flex justify-between items-start" style="margin-bottom:10px">
            <div>
              ...
              <div class="text-xs text-muted"><?= clean($c['level_name'] ?: '—') ?><?= $c['option_name'] ? ' · ' . clean($c['option_name']) : '' ?></div>
            </div>
