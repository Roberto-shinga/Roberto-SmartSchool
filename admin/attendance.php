...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$classId) {
        redirectWith($_SERVER['PHP_SELF'], 'danger', 'Classe invalide.');
    } else {
        foreach ($statuses as $studentId => $status) {
            ...
            dbExecute(
                ...
                [$studentId, $classId, $date, $status, $admin['id']]
            );
        }
        logActivity('attendance_recorded', "Classe #$classId, $date : " . count($statuses) . " eleve(s)");
        redirectWith($_SERVER['PHP_SELF'] . "?class_id=$classId&date=$date", 'success', "Presences enregistrees pour le " . formatDate($date) . ".");
    }
}
...
if ($classId) {
    $students = dbFetchAll(
        "SELECT s.id, s.student_number, COALESCE(s.first_name, u.first_name) AS fn, COALESCE(s.last_name, u.last_name) AS ln,
...
<div class="main-content">
  <div class="page-wrapper">
    ...
    <div class="page-header">
      <div><h1>Presences</h1><p>Appel par classe et par jour</p></div>
    </div>
      ...
      <div class="card-body">
        <form method="GET" class="grid-4" style="align-items:end">
          ...
          <div class="form-group" style="margin-bottom:0">
            ...
            <input type="date" name="date" class="form-control" value="<?= clean($date) ?>" onchange="this.form.submit()">
          </div>
        </form>
      </div>
    </div>
    ...
    <?php if (!$classId): ?>
      <div class="empty-state">
        <div class="empty-state-icon"><i class="bx bx-check-square"></i></div>
        <h3>Selectionne une classe</h3>
        <p>Choisis une classe et une date pour faire l'appel.</p>
      </div>
    <?php elseif (empty($students)): ?>
      <div class="empty-state">
        <div class="empty-state-icon"><i class="bx bx-user-x"></i></div>
        <h3>Aucun eleve actif dans cette classe</h3>
      </div>
    <?php else: ?>
    ...
    <div class="grid-4 mb-6">
      ...
      <div class="stat-card c-<?= $color ?>">
        <div class="stat-icon c-<?= $color ?>"><i class="bx <?= $icon ?>"></i></div>
        <div><div class="stat-value"><?= $stats[$key] ?></div><div class="stat-label"><?= $label ?></div></div>
      </div>
      <?php endforeach; ?>
    </div>
        ...
        <div class="table-wrap" style="border:none;box-shadow:none;border-radius:0">
          <table class="table">
            ...
            <tbody>
              ...
              <tr>
                <td class="td-user">
                  <div class="avatar avatar-32" style="background:var(--cyan)"><?= getInitials($s['fn'], $s['ln']) ?></div>
                  <div class="td-name"><?= clean($s['fn'] . ' ' . $s['ln']) ?></div>
                </td>
                  ...
                  <div class="att-toggle" style="display:flex;gap:6px">
                    <?php foreach ($statusMeta as $key => [$color, $icon, $label]): ?>
                      <label style="cursor:pointer">
                        ...
                        <span class="badge badge-<?= $cur === $key ? $color : 'gray' ?> att-badge" style="cursor:pointer;<?= $cur === $key ? '' : 'opacity:.45' ?>" title="<?= $label ?>">
...
function markAll(status) {
  document.querySelectorAll(`.att-radio[value="${status}"]`).forEach(r => r.closest('label').click());
}
</script>
