...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($firstName) || empty($lastName)) {
            ...
            try {
                ...
                redirectWith($_SERVER['PHP_SELF'], 'success',
                    ...
                    (APP_ENV === 'development' && !$mailOk ? " Lien (envoi email echoue) : " . BASE_URL . "/auth/activate-account.php?token=$token" : ''));
            } catch (Exception $e) {
    ...
    if ($action === 'resend') {
        ...
        if ($u && !$u['is_active']) {
            ...
            redirectWith($_SERVER['PHP_SELF'], 'success',
                "Invitation renvoyee." . (APP_ENV === 'development' && !$mailOk ? " Lien (envoi email echoue) : " . BASE_URL . "/auth/activate-account.php?token=$token" : ''));
        } else {
