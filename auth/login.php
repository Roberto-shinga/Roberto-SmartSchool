    ...
    if ($tab === 'email') {
        ...
        if (empty($login) || empty($password)) {
            ...
            if (!$user || !verifyPassword($password, $user['password'])) {
                ...
                $mailOk = send2FACodeEmail($user['email'], $user['first_name'], $code);
                logActivity('superadmin_2fa_sent', 'Code envoye pour connexion Super Admin : ' . $user['username']);
                if (APP_ENV === 'development' && !$mailOk) {
                    $_SESSION['dev_2fa_code'] = $code; // affichage debug : envoi email non configure/echoue
                }
