...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['resend'])) {
        ...
        if (time() - $lastSent < SUPERADMIN_2FA_RESEND_DELAY) {
            ...
            $mailOk = send2FACodeEmail($superAdmin['email'], $superAdmin['first_name'], $code);
            $_SESSION['pending_2fa']['last_sent'] = time();
            if (APP_ENV === 'development' && !$mailOk) $_SESSION['dev_2fa_code'] = $code;
            else unset($_SESSION['dev_2fa_code']);
  ...
  <div class="v2fa-card">
    <?php if ($devCode): ?>
      <div class="dev-banner">
        <i class="bx bx-code-alt"></i> <strong>Mode developpement</strong> — l'email n'a pas pu etre envoye (SMTP non configure dans config/mail.php, ou envoi echoue),
