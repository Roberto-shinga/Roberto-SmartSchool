        ...
        if (!$studentId || !$fee || $amount <= 0) {
            ...
            dbExecute(
                "INSERT INTO payments (student_id, fee_id, amount_paid, payment_method, receipt_number, status, collected_by, notes)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                [$studentId, $feeId, $amount, $method, generateReceiptNumber(), $status, $accountant['id'], $notes ?: null]
            );
            $paymentId = dbLastId();
...
function qsUrlAcctPay(int $page, string $q, string $status): string {
  ...
  <div class="page-wrapper">
    ...
    <div class="card">
      ...
      <div class="pagination">
        <?php if ($pg['has_prev']): ?><a href="<?= qsUrlAcctPay($pg['prev_page'], $search, $statusFlt) ?>"><i class="bx bx-chevron-left"></i></a><?php endif; ?>
        <?php for ($p = 1; $p <= $pg['total_pages']; $p++): ?>
          <?php if ($p === $pg['current_page']): ?><span class="active"><?= $p ?></span><?php else: ?><a href="<?= qsUrlAcctPay($p, $search, $statusFlt) ?>"><?= $p ?></a><?php endif; ?>
        <?php endfor; ?>
        <?php if ($pg['has_next']): ?><a href="<?= qsUrlAcctPay($pg['next_page'], $search, $statusFlt) ?>"><i class="bx bx-chevron-right"></i></a><?php endif; ?>
      </div>
      <?php endif; ?>
      <?php endif; ?>
    </div>
  ...
  <div class="modal">
    <form method="POST">
      ...
      <div class="modal-body">
        ...
        <div class="grid-2">
          <div class="form-group"><label class="form-label">Montant verse (FC) <span class="form-required">*</span></label><input type="number" name="amount_paid" class="form-control" min="1" step="1" required></div>
          <div class="form-group">
