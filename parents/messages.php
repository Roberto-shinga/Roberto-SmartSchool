...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$receiverId || empty($body)) {
        ...
        createNotification($receiverId, 'Nouveau message : ' . $subject, mb_substr($body, 0, 100), 'info', BASE_URL . '/parents/messages.php');
...
// Destinataires possibles : enseignants des classes de SES enfants + administration
$children = getParentChildren($user['id']);
$classIds = array_unique(array_column($children, 'class_id'));
$possibleRecipients = [];
if (!empty($classIds)) {
    ...
    $possibleRecipients = dbFetchAll(
        "SELECT DISTINCT u.id, u.first_name, u.last_name, u.role_id
         ...
         LEFT JOIN teacher_assignments ta ON ta.teacher_id = t.id AND ta.class_id IN ($placeholders)
         WHERE u.is_active = 1 AND (u.role_id IN (?, ?) OR ta.id IS NOT NULL)
         ORDER BY u.role_id, u.first_name",
        array_merge($classIds, [ROLE_SUPER_ADMIN, ROLE_ADMIN])
    );
} else {
    $possibleRecipients = dbFetchAll(
        "SELECT id, first_name, last_name, role_id FROM users WHERE is_active=1 AND role_id IN (?, ?) ORDER BY role_id, first_name",
      ...
      <div class="card">
        ...
        <div style="max-height:560px;overflow-y:auto">
          ...
          <?php else: foreach ($conversations as $c): ?>
            <a href="<?= BASE_URL ?>/parents/messages.php?with=<?= $c['other_id'] ?>"
