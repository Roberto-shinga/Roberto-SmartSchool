...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $subject    = sanitizeString($_POST['subject'] ?? 'Re: conversation');
    ...
    if (!$receiverId || empty($body)) {
        ...
        createNotification($receiverId, 'Nouveau message : ' . $subject, mb_substr($body, 0, 100), 'info', BASE_URL . '/teachers/messages.php');
...
// Destinataires possibles : administration + parents des eleves de ses classes + collegues
$possibleRecipients = dbFetchAll(
    "SELECT id, first_name, last_name, role_id FROM users WHERE is_active=1 AND id != ? AND role_id IN (?, ?, ?) ORDER BY role_id, first_name",
    [$user['id'], ROLE_SUPER_ADMIN, ROLE_ADMIN, ROLE_TEACHER]
);
      ...
      <div class="card">
        ...
        <div style="max-height:560px;overflow-y:auto">
          ...
          <?php else: foreach ($conversations as $c): ?>
            <a href="<?= BASE_URL ?>/teachers/messages.php?with=<?= $c['other_id'] ?>"
