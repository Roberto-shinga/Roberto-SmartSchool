...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$fee || $amount <= 0) {
        redirectWith($_SERVER['PHP_SELF'], 'danger', 'Frais et montant valides obligatoires.');
    } elseif (empty($_FILES['proof']['name'])) {
        redirectWith($_SERVER['PHP_SELF'], 'danger', 'Une preuve de paiement (photo ou PDF du recu) est obligatoire.');
    } else {
        ...
        if (!$upload['success']) {
            ...
            // Le paiement reste "en_attente" tant que l'administration n'a pas
            // valide la preuve jointe (voir admin/payments.php).
            dbExecute(
                "INSERT INTO payments (student_id, fee_id, amount_paid, payment_method, receipt_number, status, notes)
                 VALUES (?, ?, ?, ?, ?, 'en_attente', 'Declare par l\'eleve, en attente de validation')",
            ...
            logActivity('payment_declared', "Paiement declare par l'eleve : " . formatMoney($amount));
            redirectWith($_SERVER['PHP_SELF'], 'success', 'Paiement declare. Il sera valide par l\'administration apres verification.');
        }
    }
}

// ── Frais applicables a cet eleve (portee : classe > niveau > general) ──
$applicableFees = dbFetchAll(
    "SELECT f.*, fc.name AS category_name
     ...
     WHERE f.academic_year_id = ?
       AND (f.class_id = ? OR (f.class_id IS NULL AND (f.level_id = ? OR f.level_id IS NULL)))
     ORDER BY f.due_date",
    [$yearId, $student['class_id'], $student['level_id']]
);

// ── Total du, total paye, solde ─────────────────────────────────
...
<div class="main-content">
  <div class="page-wrapper">
    ...
    <div class="page-header">
      <div><h1>Mes paiements</h1><p>Frais scolaires et historique</p></div>
    ...
    <div class="card" style="margin-bottom:20px">
      ...
      <?php if (empty($applicableFees)): ?>
        <div class="card-body"><p class="text-sm text-muted">Aucun frais defini pour ta classe.</p></div>
      <?php else: ?>
    ...
    <div class="card">
      <div class="card-header"><h3><i class="bx bx-history"></i> Historique de mes paiements</h3></div>
  ...
  <div class="modal">
    <form method="POST" enctype="multipart/form-data">
      ...
      <div class="modal-body">
        <div class="alert alert-info"><i class="bx bx-info-circle"></i> Ton paiement restera "en attente" jusqu'a validation de la preuve par l'administration.</div>
